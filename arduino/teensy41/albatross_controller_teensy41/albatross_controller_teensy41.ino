...
// --- Hardware pins ---
// Teensy is the boost controller. These pins command the wastegate actuator
// power stages directly; there is no separate boost-control module in between.
// All GPIO and analog inputs must be conditioned to 0-3.3 V; Teensy 4.1 pins
// are not 5 V tolerant.
static constexpr uint8_t WG1_PWM_PIN = 2;
static constexpr uint8_t WG1_DIR_PIN = 3;
static constexpr uint8_t WG1_EN_PIN = 4;
static constexpr uint8_t WG2_PWM_PIN = 5;
...
static constexpr uint8_t WMI_FLOW_SENSOR_PIN = 20;
static constexpr uint8_t NEUTRAL_SWITCH_PIN = 25; // digital input from neutral lamp switch
// Existing bike lamp feeds must be conditioned to 3.3 V logic with external pulldowns.
static constexpr uint8_t LEFT_INDICATOR_PIN = 26;
static constexpr uint8_t RIGHT_INDICATOR_PIN = 27;
static constexpr uint8_t HIGH_BEAM_PIN = 28;
static constexpr uint8_t BRAKE_LIGHT_PIN = 29;
static constexpr uint8_t OIL_WARNING_PIN = 30;
static constexpr uint8_t WMI_PRESSURE_OK_PIN = 31;
...
static constexpr uint16_t ADC_MAX_RAW = 4095;
static constexpr uint16_t ADC_REFERENCE_MV = 3300;
static constexpr uint16_t OIL_PRESSURE_SENSOR_MIN_RAW = 413; // 0.5V sender scaled to ~0.33V at Teensy ADC
static constexpr uint16_t OIL_PRESSURE_SENSOR_MAX_RAW = 3723; // 4.5V sender scaled to ~3.0V at Teensy ADC
static constexpr uint16_t OIL_PRESSURE_SENSOR_MAX_PSI_X10 = 1000; // 100.0 psi
static constexpr uint16_t AIR_TANK_SENSOR_MIN_RAW = 413; // 0.5V sender scaled to ~0.33V at Teensy ADC
static constexpr uint16_t AIR_TANK_SENSOR_MAX_RAW = 3723; // 4.5V sender scaled to ~3.0V at Teensy ADC
...
void handleFrame(uint16_t id, uint8_t len, const uint8_t *data) {
  ...
  switch (id) {
    ...
    case CanId::POST_REQUEST: {
      uint8_t resp[2] = {0xA4, 0x01}; // device=Teensy controller, status=OK
...
uint16_t adcRawToMillivolts(int raw) {
  return static_cast<uint16_t>(constrain(map(constrain(raw, 0, ADC_MAX_RAW), 0, ADC_MAX_RAW, 0, ADC_REFERENCE_MV), 0, ADC_REFERENCE_MV));
}
...
uint8_t readWmiTankLevelPct() {
  ...
  const long scaled = map(constrain(raw, 0, ADC_MAX_RAW), 0, ADC_MAX_RAW, 0, 100);
...
void publishStatusFrames() {
  ...
  if (now - last_service_ms >= 250) {
    ...
    uint8_t sensor_voltage[8] = {
      ...
      uint8_t(ADC_REFERENCE_MV >> 8), uint8_t(ADC_REFERENCE_MV & 0xFF),
