...
void Controller::end(const Inputs& i,Reason why,bool immediate) {
  ...
  out.state=State::RECOVERY; recovery_until=i.now+c.recovery_ms;
  for(int n=0;n<4;n++) out.valve[n]=out.predicted[n]=0;
  out.demand=0; had_event=true;
}
const Outputs& Controller::update(const Inputs& i) {
  ...
  if(i.now-window>=c.budget_window_ms && out.state!=State::FIRING && out.state!=State::TAPERING) {window=i.now;used=0;}
  ...
  if(had_event && int32_t(recovery_until-i.now)>0) {
  ...
  out.accepted=true; out.profile=selectProfile(c,i,had_event);
