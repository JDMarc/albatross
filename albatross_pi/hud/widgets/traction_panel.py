    ...
    def draw(self,surface,state):
        ...
        try:
            ...
            for n,(label,level) in enumerate((("TCS",d.tcs),("AWC",d.awc))):
                ...
                surface.blit(font(level_size,bold=True).render(level_text,True,color),(x,self.rect.y+19))
            divider_x=self.rect.x+pad+half-7
            pygame.draw.line(surface,AMBER_DARK,(divider_x,self.rect.y+6),(divider_x,self.rect.y+37),1)
            ...
            status_y=max(self.rect.y+39,self.rect.bottom-21)
            pygame.draw.line(surface,AMBER_DARK,(self.rect.x+pad,status_y-3),(self.rect.right-pad,status_y-3),1)
            size=fit_font_size(status,self.rect.width-2*pad,15,start_size=12,bold=True)
            surface.blit(font(size,bold=True).render(status,True,AMBER_BRIGHT),(self.rect.x+pad,status_y))
